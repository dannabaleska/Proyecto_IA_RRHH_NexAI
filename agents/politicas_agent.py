"""
Agente RAG de Reglamento Interno.
Responde consultas sobre políticas internas utilizando Gemini y ChromaDB.
"""
...
NOMBRE_AGENTE = "Agente de Políticas Internas"

# Rutas relativas al proyecto (nunca absolutas, según el estándar del equipo)
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
RUTA_CONOCIMIENTO = os.path.join(BASE_DIR, "knowledge", "02_Reglamento_Interno.txt")
RUTA_VECTORSTORE = os.path.join(BASE_DIR, "vectorstores", "politicas")
NOMBRE_COLECCION = "politicas_internas"
...
PROMPT_POLITICAS = """Eres el {agente} de Patito S.A.
Respondes preguntas sobre jornada laboral, vacaciones, permisos, código de conducta y sanciones.

Reglas estrictas:
- Responde ÚNICAMENTE con base en el CONTEXTO entregado.
- Si la información no está en el contexto, responde exactamente:
"No tengo esa información en el reglamento interno."
...
def responder(pregunta: str) -> dict:
    ...
    prompt_final = PROMPT_POLITICAS.format(contexto=contexto, pregunta=pregunta)
...
if __name__ == "__main__":
    pregunta_prueba = "¿Cuántos días de vacaciones tengo?"
